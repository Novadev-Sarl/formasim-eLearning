...
export const useAuthStore = defineStore('auth', () => {
  ...
  const logout = () => {
    ...
    // If the user is on a protected page, redirect to the login page
    if (authenticatedPages.includes(router.currentRoute.value.path)) {
      router.push('/login')
    }
  }

  /**
   * Make a call to the API to update the user's password.
   * @param input - The input to update the password.
   */
